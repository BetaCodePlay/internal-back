...
class TransactionsCollection
{
     ...
     * @param array $transactions Transactions data
     */
    public function formatDepositsAndWithdrawals($transactions)
    {
        $transactionsData = [];
        $total = 0;
        ...
        foreach ($transactions as $transaction) {
            ...
            $transactionObject->user = sprintf(
                '<a href="%s" class="btn u-btn-3d u-btn-primary btn-sm" target="_blank">%s</a>',
                route('users.details', $transaction->user),
                $transaction->user
            );
            $transactionObject->username = $transaction->username;
            $transactionObject->amount = number_format($transaction->amount, 2);
            $transactionObject->created = $transaction->created_at->setTimezone($timezone)->format('d-m-Y H:i:s');
            $transactionObject->updated = $transaction->updated_at->setTimezone($timezone)->format('d-m-Y H:i:s');
            $transactionObject->description = Providers::getDescription($transaction->provider_id, $transaction->transaction_type_id, $action = null, $transaction->data);
            $transactionObject->currency_iso = $transaction->currency_iso;

            if (isset($transaction->data->operator)) {
                $transactionObject->operator = $transaction->data->operator;
            } else {
                $transactionObject->operator = null;
            }
            ...
            if (!is_null($transaction->details) || !is_null($transaction->data)) {
                switch ($transaction->provider_id) {
                    case Providers::$vcreditos_api:
                    {
                        ...
                        $transactionObject->info = $vcreditosApi;
                        break;
                    }
                    case Providers::$just_pay:
                    {
                        ...
                        $transactionObject->info = $justPay;
                        break;
                    }
                    default:
                    {
                        $transactionObject->info = '';
                        break;
                    }
                }
            } else {
            ...
            $transactionObject->status = $statusText;
            $transactionObject->provider = Providers::getName($transaction->provider_id);
            $transactionObject->currency = $transaction->currency_iso;

            $transactionsData[] = $transactionObject;
        }
        return [
