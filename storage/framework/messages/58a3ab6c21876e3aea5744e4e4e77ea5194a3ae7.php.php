...
use Dotworkers\Bonus\Repositories\UsersRepo;
...
class Bonus
{
     ...
     */
    public static function assignBonus($campaign, $bonus, $wallet, $walletToken, $user, $currency, $whitelabel, $operator = null)
    {
     ...
     * @param string $code Bonus code
     * @param null|double $rolloverAmount Initial rollover amount
     */
    private static function bonusCode($whitelabel, $currency, $user, $wallet, $walletToken, $campaignId, $rolloverAmount = null, $operator = null)
    {
        if (Configurations::getBonus($whitelabel)) {
            $campaignsRepo = new CampaignsRepo();
            $rolloversRepo = new RolloversRepo();
        ...
        if (Configurations::getBonus($whitelabel)) {
            ...
            if (!is_null($campaignData)) {
                ...
                switch ($campaignData->allocation_criteria_id) {
                    case AllocationCriteria::$deposit:
                    {
                        $transactionsRepo = new TransactionsRepo();
                        $firstDeposit = $transactionsRepo->findFirstDeposit($user, $currency);
                        self::welcomeBonusWithDeposit($campaignData->id, $whitelabel, $currency, $user, $firstDeposit->amount, $wallet, $walletAccessToken, $rolloverAmount);
                        break;
                    }
                    case AllocationCriteria::$registration:
                    {
                        self::welcomeBonusWithoutDeposit($whitelabel, $currency, $user, $wallet, $walletAccessToken, $campaignData->id, $rolloverAmount);
                        break;
                    }
                    case AllocationCriteria::$next_deposit_bonus:
                    {
                        ...
                        self::nextDeposit($campaignData->id, $whitelabel, $currency, $user, $transactionsApproved->amount, $wallet, $walletAccessToken, $rolloverAmount);
                        break;
                    }
                }
    ...
    private static function depositBonus($whitelabel, $currency, $user, $deposit, $allocationCriteria, $wallet, $walletToken, $campaignId, $rolloverAmount = null, $operator = null)
    {
        if (Configurations::getBonus($whitelabel)) {
            ...
            if (self::activeCampaign($campaign) && $campaign->status) {
                ...
                if ($allocationCriteria == AllocationCriteria::$welcome_bonus_with_deposit || $allocationCriteria == AllocationCriteria::$bonus_code_with_deposit || $allocationCriteria == AllocationCriteria::$next_deposit_bonus) {
    ...
    public static function tournament($whitelabel, $currency, $user, $game, $amount, $provider)
    {
        if (Configurations::getBonus($whitelabel)) {
            ...
            if (self::activeCampaign($campaign) && $campaign->status) {
                ...
                if ($campaignData->amount_min = '*' && $campaignData->amount_max = '*' ||
                        $campaignData->amount_min = '*' && $campaignData->amount_max <= $amount ||
                            $campaignData->amount_min >= $amount && $campaignData->amount_max = '*' ||
                                $campaignData->amount_min >= $amount && $campaignData->amount_max <= $amount
                ) {
    ...
    /**
     * Active bonus for code with deposit
     *
     * @param int $whitelabel Whitelabel ID
     * @param string $currency Currency ISO
     * @param string $user User ID
     ...
     * @param string $operator Operator ID
     */
    public static function depositWithCode($campaign, $whitelabel, $currency, $user, $deposit, $wallet, $walletToken, $rolloverAmount = null, $operator = null)
    {
        if (Configurations::getBonus($whitelabel)) {
            $campaignsRepo = new CampaignsRepo();
            $campaign = $campaignsRepo->find($campaign);

            if (self::activeCampaign($campaign) && $campaign->status) {
                self::depositBonus($whitelabel, $currency, $user, $deposit, AllocationCriteria::$bonus_code_with_deposit, $wallet, $walletToken, $campaign->id, $rolloverAmount, $operator);
            }
        }
    }
    ...
    /**
     * Activate bonus for the next deposit
     *
     * @param int $campaign Campaign ID
     * @param int $whitelabel Whitelabel ID
     * @param string $currency Currency ISO
     ...
     * @param string $operator Operator ID
     */
    public static function nextDeposit($campaign, $whitelabel, $currency, $user, $deposit, $wallet, $walletToken, $rolloverAmount = null, $operator = null)
    {
        self::depositBonus($whitelabel, $currency, $user, $deposit, AllocationCriteria::$next_deposit_bonus, $wallet, $walletToken, $campaign, $rolloverAmount, $operator);
    }
    ...
    /**
     * Activate welcome bonus with deposit
     *
     * @param int $campaign Campaign ID
     * @param int $whitelabel Whitelabel ID
     * @param string $currency Currency ISO
     ...
     * @param string $operator Operator ID
     */
    public static function welcomeBonusWithDeposit($campaign, $whitelabel, $currency, $user, $deposit, $wallet, $walletToken, $rolloverAmount = null, $operator = null)
    {
        self::depositBonus($whitelabel, $currency, $user, $deposit, AllocationCriteria::$welcome_bonus_with_deposit, $wallet, $walletToken, $campaign, $rolloverAmount, $operator);
    }

    /**
     * Activate welcome bonus without deposit
     *
     * @param int $whitelabel Whitelabel ID
     * @param string $currency Currency ISO
     * @param string $user User ID
     ...
     * @param null|double $rolloverAmount Initial rollover amount
     */
    public static function welcomeBonusWithoutDeposit($whitelabel, $currency, $user, $wallet, $walletToken, $campaign, $rolloverAmount = null)
    {
        self::depositBonus($whitelabel, $currency, $user, $deposit = null, AllocationCriteria::$welcome_bonus_without_deposit, $wallet, $walletToken, $campaign, $rolloverAmount, $operator = null);
    }
}
