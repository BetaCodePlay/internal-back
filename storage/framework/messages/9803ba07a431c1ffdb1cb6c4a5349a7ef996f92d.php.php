        ...
        try {
            if (!is_null($startDate) && !is_null($endDate)) {
                ...
                foreach ($agents as $agent) {
                    $agentsId[] = $agent->user_id;
                }
                $providers = [Providers::$agents, Providers::$agents_users];
                $startDate = Utils::startOfDayUtc($startDate);
                $endDate = Utils::endOfDayUtc($endDate);
                $transactions = $this->transactionsRepo->getAgentsTransactions($agentsId, $providers, $currency, $startDate, $endDate);
            } else {
                $transactions = [];
            }
            ...
            if ($id != $user) {
                ...
                if ($type == 'user') {
                    ...
                    if ($status == Status::$ok) {
                        ...
                        if ($balance == 0 || $ownerBalance == 0 && $ownerAgent->username != 'support') {
                            \Log::debug(__METHOD__, ['operation' => $transactionType, 'amount' => $amount, 'owner_amount' => $ownerBalance, 'agent_amount' => $agentBalance, 'new_balance' => $balance, 'transaction' => $ticket]);
                        }
