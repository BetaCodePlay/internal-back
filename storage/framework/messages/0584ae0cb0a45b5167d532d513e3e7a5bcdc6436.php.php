...
class CampaignsCollection
{
    ...
    public function formatCampaignByCriteria($campaigns, $convert, $campaignStartDate, $campaignEndDate): array
    {
        ...
        foreach ($campaigns as $campaign) {
            ...
            $usedBonuses = $campaignParticipationDetailsRepo->getByCampaignAndStatus($campaign->id, [CampaignParticipationStatus::$in_use]);
            $endedBonuses = $campaignParticipationDetailsRepo->getByCampaignAndStatus($campaign->id, [CampaignParticipationStatus::$canceled_by_user, CampaignParticipationStatus::$canceled_by_administrator, CampaignParticipationStatus::$canceled_by_withdrawal, CampaignParticipationStatus::$completed_rollover, CampaignParticipationStatus::$expired_rollover]);
            $convertedBonuses = $campaignParticipationDetailsRepo->getByCampaignAndStatus($campaign->id, [CampaignParticipationStatus::$completed_rollover]);
            ...
            $participation = $campaignParticipationRepo->getTotalParticipation($campaign->id, $campaignStartDate, $campaignEndDate);
            $activeUsers = $campaignParticipationRepo->getTotalParticipationAdStatus($campaign->id, [CampaignParticipationStatus::$in_use, CampaignParticipationStatus::$assigned], $campaignStartDate, $campaignEndDate);
            ...
            foreach ($activeBonuses->data->bonus as $activeBonus) {
                if ($activeBonus->campaign_id == $campaign->id) {
            ...
            foreach ($allocationCriteriaMet as $criteriaMet) {
                if ($criteriaMet->campaign_id == $campaign->id) {
                    $totalCriteriaMet += $criteriaMet->criteria_met_quantity;
                }
            }

            $campaign->criteria = AllocationCriteria::getName($campaign->allocation_criteria_id);
            ...
            $startDate = $campaign->start_date->setTimezone($timezone)->format('d-m-Y h:i:s');
            $endDate = !is_null($campaign->end_date) ? $campaign->end_date->setTimezone($timezone)->format('d-m-Y') : _i('No end date');
            ...
            if (isset($campaign->data->promo_codes) && $campaign->data->promo_codes != []) {
                foreach ($campaign->data->promo_codes as $promo_codes)
    ...
    public function formatCampaignParticipation($users, $convert)
    {
        ...
        foreach ($users as $user) {
            ...
            $usedBonuses = $campaignParticipationDetailsRepo->getByCampaignStatusAndUser($user->campaign_id, [CampaignParticipationStatus::$in_use], $user->user_id);
            $endedBonuses = $campaignParticipationDetailsRepo->getByCampaignStatusAndUser($user->campaign_id, [CampaignParticipationStatus::$canceled_by_user, CampaignParticipationStatus::$canceled_by_administrator, CampaignParticipationStatus::$canceled_by_withdrawal, CampaignParticipationStatus::$completed_rollover, CampaignParticipationStatus::$expired_rollover], $user->user_id);
            $convertedBonuses = $campaignParticipationDetailsRepo->getByCampaignStatusAndUser($user->campaign_id, [CampaignParticipationStatus::$completed_rollover], $user->user_id);
            ...
            foreach ($activeBonuses->data->bonus as $activeBonus) {
                if ($activeBonus->campaign_id == $user->campaign_id && $activeBonus->user_id == $user->user_id) {
            ...
            $user->criteria = AllocationCriteria::getName($user->allocation_criteria_id);
