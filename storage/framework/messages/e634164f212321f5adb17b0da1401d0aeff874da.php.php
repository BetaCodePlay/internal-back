...
class CampaignsRepo
{
     ...
     * @return mixed
     */
    public function getByAllocationCriteria($whitelabel, $criteria, $campaignID, $currency)
    {
        $campaign = Campaign::select('campaigns.*', 'campaign_allocation_criteria.name as criteria', 'rollover_types.provider_type_id')
     ...
     * @return mixed
     */
    public function getStatusAndAllocationCriteriaAndCurrency($allocationCriteria, $currency)
    {
        ...
        if ($allocationCriteria != '*' && !empty($allocationCriteria)) {
            $campaigns->where('campaigns.allocation_criteria_id', $allocationCriteria);
        }

        if ($currency != '*' && !empty($currency)) {
            $campaigns->where('campaigns.currency_iso', $currency);
        }
