...
class Providers
{
     ...
     * @var int
     */
    public static $gateway_betsweet = 175;
     ...
     * @var int
     */
    public static $mercado_pago = 176;
    ...
    public static function getDescription($provider, $transactionType, $action, $data)
    {
        ...
        switch ($provider) {
            ...
            {
                if (is_null($data)) {
                    $description = _i('Bet on the game');
                } else {
                    $type = (isset($data->bet_type) ? $data->bet_type : $data->type);
                    ...
                    switch ($type) {
                        ...
                        case 4:
                        {
                            $description = _i('Freespin #%s on the game %s', [$data->provider_transaction, $data->game,]);
                            break;
                        }
                        case 5:
                        {
            ...
            case self::$betsoft:
            {
                switch ($data->type) {
                    ...
                    {
                        if ($data->refund) {
                            $description = _i('Bet #%s refunded', [$data->provider_transaction]);
            ...
            case self::$iq_soft:
            {
                ...
                switch ($type) {
                    ...
                    {
                        if ($transactionType == TransactionTypes::$credit) {
                            $description = _i('Rollback prize #%s', [$data->provider_transaction]);
                        } else {
            ...
            case self::$pragmatic_play_virtual:
            {
                ...
                if (isset($data->bet_type)) {
                    switch ($data->bet_type) {
                        ...
                        case 'promo':
                        {
                            $description = _i('Prize won on the game %s', [$providerTransaction, $data->game,]);
                            break;
                        }
                    }
                } else {
            ...
            case self::$pragmatic_play_live_casino:
            {
                ...
                switch ($data->type) {
                    case 'debit':
                    {
                        $description = _i('Bet in round #%s on the game %s', [$providerTransaction, $data->game,]);
                        break;
                    }
                    case 'credit':
                    {
                        $description = _i('Bet in round #%s won on the game %s', [$providerTransaction, $data->game,]);
                        break;
                    }
                    case 'rollback':
                    {
                        $description = _i('Rollback bet in round #%s on the game %s', [$providerTransaction, $data->game,]);
                        break;
                    }
                }
                break;
            ...
            case self::$geritus:
            {
                if ($transactionType == TransactionTypes::$credit) {
                    ...
                    if(isset($data->productId)) {
                        $description .= " Product Id: $data->productId";
                    }
                } else {
                    if (isset($data->bet_type)) {
                        switch ($data->bet_type) {
                            ...
                            {
                                $providerTransaction = isset($data->event) ? $data->event : $data->provider_transaction;
                                $description = _i('Rollback bet #%s won on the game %s', [$providerTransaction, $data->game,
                                ]);
                                break;
                            }
                        }
            ...
            case self::$binance:
            {
                if ($transactionType == TransactionTypes::$credit) {
                    switch ($action) {
                        case Actions::$generic:
                        {
                            $description = _i('Deposit via binance');
                            break;
                        }
                        case Actions::$unlock:
                        {
                    ...
                    switch ($action) {
                        case Actions::$lock:
                        {
                            $description = _i('Balance locked for withdrawal processing via binance');
                            break;
                        }
                        case Actions::$unlock:
                        {
            ...
            case self::$personal:
            {
                if ($transactionType == TransactionTypes::$credit) {
                    switch ($action) {
                        case Actions::$generic:
                        {
                            $description = _i('Deposit via personal');
                            break;
                        }
                        case Actions::$unlock:
                        {
            ...
            case self::$charging_point:
            {
                if ($transactionType == TransactionTypes::$credit) {
                    ...
                    $description = _i('Withdrawal via charging point. Code %s', $data->reference);
                }
                break;
            }
            case self::$nequi:
            ...
            {
                if ($transactionType == TransactionTypes::$credit) {
                    $description = _i('Deposit via MercadoPago');
                } else {
                    $description = _i('Withdrawal via MercadoPago');
                }
                break;
            }
            case self::$pix:
            {
                if ($transactionType == TransactionTypes::$credit) {
                    $description = _i('Deposit via Pix');
                } else {
                    $description = _i('Withdrawal via Pix');
                }
                break;
            }
            default:
            {
                \Log::info('Provider description not configured', ['provider' => $provider, 'data' => $data,]);
                break;
            }
        }
        return $description;
    ...
    public static function getName(int $provider)
    {
        ...
        switch ($provider) {
            ...
            default:
            {
                \Log::info('Provider name not configured', ['provider' => $provider]);
                break;
            }
        }
        return $name;
