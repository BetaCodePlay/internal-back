...
//        $whitelabel = Configurations::getWhitelabel();
     ...
     * @throws \Illuminate\Validation\ValidationException
     */
    public function performTransactions(Request $request, TransactionsRepo $transactionsRepo, AgentsRepo $agentsRepo,AgentCurrenciesRepo $agentCurrenciesRepo)
    {
        $this->validate($request, [
        ...
        try {
            ...
            switch ($request->type){
                ...
                case 'agent':{
                    $result = AgentsController::performTransactionsAgent($request, $agentsRepo, $agentCurrenciesRepo, $transactionsRepo);
                    Log::notice('performTransactionsAgent',[$result,'performTransactionsAgent']);
                    return $result;
                    break;
                }
        ...
        try {
            ...
            switch ($transactionType){
                case TransactionTypes::$credit:{
                    ...
                    $balance = $agentsRepo->getAndUpdateBalance($currency,$userAuth->id,$userAffected,$amount);

                    /*error debiting*/
                    if(isset($balance[0]->status) && !$balance[0]->status){
                        $data = [
                            'title' => _i('Insufficient balance'),
                            'message' => _i("The agents's operational balance is insufficient to perform the transaction"),
                            'close' => _i('Close')
                ...
                case TransactionTypes::$debit:{
                    ...
                    $balance = $agentsRepo->getAndUpdateBalance($currency,$userAffected,$userAuth->id,$amount);
